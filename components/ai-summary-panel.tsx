...
export function AiSummaryPanel({ tree, normalizedDocument }: AiSummaryPanelProps) {
  ...
  const documentRef = useRef<NormalizedDocument | null>(normalizedDocument ?? null);
  const instanceKey = `${tree.id}:${tree.meta.createdAt}`;

  useEffect(() => {
    documentRef.current = normalizedDocument ?? null;
  }, [normalizedDocument]);

  const requestSummary = useCallback(async (reason: 'auto' | 'manual') => {
    const currentDocument = documentRef.current;
    if (!currentDocument) {
      setLoading(false);
      setError('原文快照缺失，无法生成文档摘要');
      return;
    }
    ...
    try {
      const res = await fetch('/api/summary', {
        ...
        body: JSON.stringify({ normalizedDocument: currentDocument }),
      ...
      const nextPoints = Array.isArray(json.points)
        ? json.points.filter((item) => typeof item === 'string' && item.trim())
        : [];
  ...
  }, [instanceKey, normalizedDocument, requestSummary]);
      ...
      <article className="ai-summary-card">
        <header className="ai-summary-head">
          ...
          <button
            ...
            disabled={loading || !normalizedDocument}
            title="重新生成"
            aria-label="重新生成摘要"
          >
            ↻
          ...
          <div className="ai-summary-foot-left">
            <span className="ai-summary-source-text">
              {stale
                ? '原文已更新，正在同步摘要...'
                : updatedAt
                  ? `基于原文内容生成 · ${formatTime(updatedAt)}`
                  : '基于原文内容生成'}
            </span>
            {error && points.length > 0 ? <span className="ai-summary-error-inline">{error}</span> : null}
